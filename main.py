...
def draw(win, paddle, ball, bricks, lives):
    ...
    lives_text = LIVES_FONT.render(f"Lives: {lives}", 1, "green")
    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col, row * brick_height +
                          gap * row, brick_width, brick_height, 2, [(255, 255, 0), (255, 0, 0)])
            bricks.append(brick)
...
def main():
    ...
    paddle = Paddle(paddle_x, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, "#ADD8E6")
